...
enum UsageMapper {
    ...
    /// Maps organization response to organization ID
    /// - Parameter dto: Organization response DTO
    /// - Returns: Organization ID or nil
    /// - Throws: Domain error if no organization found
    static func toOrganizationId(_ dto: OrganizationResponseDTO) throws -> OrganizationId {
        guard let firstMembership = dto.memberships.first else {
            throw DomainError.organizationNotFound
        }

        return try OrganizationId.create(firstMembership.organization.uuid)
    }
}
