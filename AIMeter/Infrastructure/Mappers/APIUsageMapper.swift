...
enum APIUsageMapper {
    ...
    /// Maps organization response to OrganizationId
    /// - Parameter response: Organization API response
    /// - Returns: OrganizationId
    /// - Throws: DomainError if no organization found
    nonisolated static func toOrganizationId(_ response: OrganizationAPIResponse) throws -> OrganizationId {
        guard let membership = response.memberships?.first else {
            throw DomainError.organizationNotFound
        }

        return try OrganizationId.create(membership.organization.uuid)
    }
