...
public struct OAuthCredentials: Codable, Sendable, Equatable {
    ...
    public nonisolated func toSessionKey() throws -> SessionKey {
        try SessionKey.create(accessToken)
    }

    /// Create updated credentials after refresh
    public func withRefreshedTokens(
        accessToken: String,
        refreshToken: String,
        expiresIn: Int
    ) -> OAuthCredentials {
...
extension OAuthCredentials {
    ...
    public static func fromClaudeCodeJSON(_ json: [String: Any]) throws -> OAuthCredentials {
    ...
    public func toClaudeCodeJSON() -> [String: Any] {
...
public enum OAuthCredentialsError: LocalizedError, Equatable {
