...
public final class GentleUpdateDelegate: NSObject, SPUStandardUserDriverDelegate {
    ...
    public func standardUserDriverWillHandleShowingUpdate(
        ...
        let updateLevel = UIConstants.WindowLevel.updateAlert
        DispatchQueue.main.async {
            NSApp.setActivationPolicy(.regular)
            NSApp.activate()

            // Raise Sparkle windows above Settings (which uses WindowLevel.settings)
            for window in NSApp.windows {
                let className = String(describing: type(of: window))
                if className.hasPrefix("SPU") || className.contains("Sparkle") {
            ...
            // Retry after delay — Sparkle may not have created its window yet
