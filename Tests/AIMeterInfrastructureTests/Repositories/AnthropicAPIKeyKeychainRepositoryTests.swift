...
/// Tests for AnthropicAPIKeyKeychainRepository — CRUD operations via KeychainService.
@Suite("AnthropicAPIKeyKeychainRepository")
struct AnthropicAPIKeyKeychainRepositoryTests {
    ...
    func saveCallsKeychainSave() async throws {
        ...
        let repo = AnthropicAPIKeyKeychainRepository(keychainService: mockKeychain)
        let key = try AnthropicAPIKey.create("sk-ant-api03-abc123def456789xyz")
        ...
        #expect(await mockKeychain.lastSavedValue == "sk-ant-api03-abc123def456789xyz")
        #expect(await mockKeychain.lastSavedKey == "anthropicAPIKey")
    }
    ...
    func getReturnsKeyWhenValidValueStored() async {
        // Arrange
        let mockKeychain = MockKeychainService()
        await mockKeychain.configure(readResult: "sk-ant-api03-abc123def456789xyz")
        let repo = AnthropicAPIKeyKeychainRepository(keychainService: mockKeychain)
        ...
        // Assert
        #expect(result?.value == "sk-ant-api03-abc123def456789xyz")
        #expect(await mockKeychain.readCallCount == 1)
    }

    @Test("get returns nil when stored value is invalid key format")
    func getReturnsNilWhenStoredValueInvalid() async {
        // Arrange — stored value does not pass AnthropicAPIKey.create validation
        let mockKeychain = MockKeychainService()
        ...
        // Assert — AnthropicAPIKey.create throws, so get returns nil
        #expect(result == nil)
    }
    ...
    func getReadsFromCorrectKeychainKey() async {
        ...
        // Assert
        #expect(await mockKeychain.lastReadKey == "anthropicAPIKey")
    }
    ...
    func deleteCallsKeychainDelete() async {
        ...
        #expect(await mockKeychain.lastDeletedKey == "anthropicAPIKey")
    }
    ...
    func existsChecksCorrectKeyName() async {
        ...
        // Assert
        #expect(await mockKeychain.lastExistsKey == "anthropicAPIKey")
    }
    ...
    func saveThenGetReturnsStoredKey() async throws {
        // Arrange — use an in-memory mock that stores values
